...
public:
    ...
    singledog(std::string name):Node(name)
    {
        ...
        //  3.创建发布者订阅者
        pub_test = this->create_publisher<cpp_my_interfaces::msg::Test>("yes_or_not", 10);
        sub_test = this->create_subscription<cpp_my_interfaces::msg::Test>(
            "my_axis", 10, std::bind(&singledog::meg_callback, this, _1));
...
private:
    ...
    void meg_callback(const cpp_my_interfaces::msg::Test::SharedPtr msg)
    {
        ...
        if(msg->flag.data)
        {
            RCLCPP_INFO(this->get_logger(),"哈哈哈,(%d,%d)的人也是单身狗!\n",msg->x1.data,msg->y1.data);
        }
        else
        {
    ...
    {
        //  回调函数处理
        this->axis.flag.set__data(true);
        this->axis.x1.data = 2;
        this->axis.y1.data = 6;
        this->axis.msgs.data = "我是一个单身狗呜呜呜！";
...
int main(int argc, char ** argv)
{
    ...
    //  实例化继承了Node的类
    auto node = std::make_shared<singledog>("duan");
    //  spin循环节点
    rclcpp::spin(node);
    //  关闭客户端库
    rclcpp::shutdown();
